...
This DAG calculates raw average Bitcoin price from the hourly data
collected by the binance_calculate_hourly DAG.

Schedule: Runs every 24 hours (raw)
Reads: /data/binance/hourly/{date}/hourly_avg.csv
Writes: /data/binance/raw/daily_avg.csv
"""
...
def _calculate_daily_average(**context):
    """
    Calculates raw average price from hourly data.
    ...
    # Path to hourly data
    hourly_file = Path(f"airflow_home/dags/data-pipelines-cu-main/lecture3/data/binance/hourly/{current_date}/hourly_avg.csv")
    ...
    try:
        ...
        # Save to CSV
        output_dir = Path("airflow_home/dags/data-pipelines-cu-main/lecture3/data/binance/daily")
        ...
        if output_file.exists():
            ...
            print(f"Created new raw average file: {output_file}")
            return daily_stats
        ...
        print(f"Error calculating raw average: {e}")
        raise
...
dag = DAG(
    ...
    description="Calculates raw average Bitcoin price from hourly data",
    schedule=timedelta(days=1),  # Run every 24 hours
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["binance", "crypto", "price", "raw", "aggregation"],
    default_args={
        'retries': 2,
